...
import { fileExistsAtPath } from "./CheckpointUtils.js";
import { getLfsPatterns, writeExcludesFile } from "./CheckpointExclusions.js";
import type { CheckpointAddResult } from "../types.js";

/**
 * GitOperations Class
 *
 * Handles git-specific operations for the Checkpoint system.
 *
...
export class GitOperations {
  ...
  public async initShadowGit(gitPath: string, cwd: string, taskId: string): Promise<string> {
    console.info(`Initializing shadow git for task ${taskId}`);
    ...
    await git.addConfig("user.name", "MCP Checkpoint");
    await git.addConfig("user.email", "checkpoint@mcp.local");
    ...
    console.info(`Shadow git initialization completed in ${durationMs}ms`);
   ...
   * The root .git directory is preserved. Uses globby to find nested .git directories and
